...
export const LOB_SCHEMAS = {
  restaurant: {
    ...
    templateExamples: [
      ...
      ['M004', 'Mixture (250g)', 'Readymade savory — packing counter', 80, 'Savories', 'all', '', 'TRUE', 0, 1, 0, 'sweets_counter', 2, 'TRUE', 'main'],
    ],
    columnHelp: [
      ['Column guide - RESTAURANT / Cloud Kitchen / Tiffin (NOT food products)'],
      [''],
      ['This template is different from Packaged Food / Home Baker.'],
      ['Restaurant = prepared dishes. Use is_available TRUE/FALSE only (no is_stocked column, no current_stock).'],
      ['TRUE marks the item in stock for the kitchen / WhatsApp menu. FALSE marks it out of stock.'],
      ['Do NOT use a food-products file here. Food products use current_stock + Record batch for jar/batch quantities.'],
      ['Sweets & Savories at a restaurant still use this restaurant template; optional jar counts use Record batch in the app, not Excel qty replace.'],
      [''],
      ['id - optional. Leave blank to autogenerate. Colliding ids are kept/reassigned safely (no duplicate-key failures).'],
      ['is_available - TRUE or FALSE (required for kitchen open). Maps to both availability and stock toggle.'],
      ...
      ['kitchen_station - cooked: tawa, steamer, kadai, beverages, assembly, cold; readymade Sweets & Savories: sweets_counter (→ packing). Blank on re-upload keeps current station. Blank/assembly + Sweets/Savories → sweets_counter.'],
    ...
    previewColumns: [
      { key: 'id', label: 'ID', mono: true, width: '7%' },
      { key: 'name', label: 'Name', bold: true, width: '18%' },
      { key: 'category', label: 'Category', width: '11%' },
      { key: 'kitchen_station', label: 'Station', mono: true, width: '11%' },
      { key: 'time_slot', label: 'Slot', width: '10%' },
      { key: 'price', label: 'Price', price: true, width: '7%' },
  ...
  food_products: {
    ...
    templateColWidths: [
      ...
      { wch: 12 }, { wch: 12 }, { wch: 28 }, { wch: 22 }, { wch: 28 }, { wch: 28 },
      { wch: 12 },
      { wch: 28 },
    ...
    templateExamples: [
      ['MP-250', 'Mango Pickle', 'Traditional Andhra-style mango pickle', 150, 'Pickles', '', 'TRUE', 'PRODUCT', 'MANGO-PICKLE', '250g', 250, 50, '', '', '', 90, '2026-07-15', 'Mango, chilli, mustard oil, salt', 'Mustard', 'Mix with rice or dosa', 'Cool dry place; refrigerate after opening', 45, '', '', '', '', '', 20, 7, 5],
      ['MP-500', 'Mango Pickle', 'Traditional Andhra-style mango pickle', 280, 'Pickles', '', 'TRUE', 'PRODUCT', 'MANGO-PICKLE', '500g', 500, 40, '', '', '', 90, '2026-07-15', 'Mango, chilli, mustard oil, salt', 'Mustard', 'Mix with rice or dosa', 'Cool dry place; refrigerate after opening', 60, '', '', '', '', '', '', '', 5],
      ['MP-1KG', 'Mango Pickle', 'Traditional Andhra-style mango pickle', 520, 'Pickles', '', 'TRUE', 'PRODUCT', 'MANGO-PICKLE', '1kg', 1000, 20, '', '', '', 90, '2026-07-15', 'Mango, chilli, mustard oil, salt', 'Mustard', 'Mix with rice or dosa', 'Cool dry place; refrigerate after opening', 90, '', '', '', '', '', '', '', 5],
      ['MP-100', 'Mango Pickle', '100g jar for samplers', 70, 'Pickles', '', 'TRUE', 'PRODUCT', 'MANGO-PICKLE', '100g', 100, 100, '', '', '', 90, '2026-07-15', 'Mango, chilli, mustard oil, salt', 'Mustard', 'Mix with rice or dosa', 'Cool dry place; refrigerate after opening', 30, '', '', '', '', '', '', '', 10],
      ['NEW-GINGER', 'Ginger Pickle (launch)', 'Batch cooking next week', 180, 'Pickles', '', 'TRUE', 'PRODUCT', '', '250g', 250, '', 'coming_soon', '2026-08-01', 50, 90, '', 'Ginger, chilli, mustard oil', 'Mustard', 'Best with hot rice', 'Cool dry place', 45, '', '', '', '', '', '', '', 5],
      ['HAMPER-PICKLE-3', 'Pickle Sampler (3×100g)', 'Three favourite pickles in travel jars', 249, 'Hampers', '', 'TRUE', 'BUNDLE', '', '3×100g', 300, 15, '', '', '', 90, '2026-07-15', '', '', '', 'Cool dry place', '', 'MP-100:3', '', '', '', '', '', '', 5],
    ],
    columnHelp: [
      ['Column guide - PACKAGED FOOD / Home Baker (NOT restaurant)'],
      [''],
      ['This template is different from Restaurant / Cloud Kitchen / Tiffin.'],
      ['Food products track batch quantities with current_stock + Record batch in the app.'],
      ['Do NOT use this file for a restaurant menu. Restaurants use is_available only (no current_stock column).'],
      [''],
      ...
      ['days_to_empty - optional days until a typical unit runs out (enables WhatsApp refill reminders when enabled in Settings)'],
      ...
      ['is_available - TRUE / FALSE (also gated by current_stock and FSSAI for food_products)'],
    ],
    previewColumns: [
    ...
    mapRow(row) {
      ...
      return {
        ...
        days_to_empty: (() => {
          const raw = row['days_to_empty'] ?? row['Days To Empty'] ?? row['days to empty'];
          if (raw == null || raw === '') return null;
