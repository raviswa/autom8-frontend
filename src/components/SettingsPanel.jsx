 ...
 *
 * Tabs:
 *   Tables / Restaurant / Services / Kitchen / Promotions / Staff / Brand
 *   WhatsApp lives on My Account (redirect from ?tab=whatsapp)
 *
...
function TabStaff({ apiClient, showToast, lobType = 'restaurant' }) {
  ...
  const saveEdit = async (emp) => {
    ...
    setEditSaving(true);
    try {
      const isSelfOwner = emp.role === 'owner' && emp.id === user?.id;
      const elevating = editForm.role !== emp.role && ['manager', 'owner'].includes(editForm.role);
      const ownerPhoneChanging = !isSelfOwner && emp.role === 'owner'
        && String(editForm.phone || '').replace(/\D/g, '') !== String(emp.phone || '').replace(/\D/g, '');
      const ownerEmailChanging = !isSelfOwner && emp.role === 'owner'
  ...
  const removeEmployee = async (emp) => {
    ...
    if (isAlertNumber) {
      const proceed = window.confirm(
        `${emp.full_name}'s WhatsApp is the outlet Manager phone (My Account → WhatsApp).\n\n` +
    ...
    try {
      ...
      if (res.data?.manager_phone_cleared) {
        setManagerPhone('');
        showToast(`${emp.full_name} removed. Manager phone cleared — set a new one under My Account → WhatsApp.`, 'error');
      } else {
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16, flexWrap: 'wrap', gap: 8 }}>
        <div>
          ...
          <div style={{ fontSize: 11, color: C.textMuted, marginTop: 2 }}>
            {restaurantLob
              ? 'Captains get takeaway WhatsApp alerts. Managers/owners get ops alerts via My Account → Manager phone plus their Team WhatsApp. Kitchen and wait staff use the kitchen display only.'
              : 'Managers/owners get ops alerts via My Account → Manager phone plus their Team WhatsApp. Packing and Dispatch staff use the packing display. Sales staff use the manager portal.'}
          </div>
        </div>
        ...
        <div style={{ ...{background: C.primaryLight, border: `0.5px solid ${C.primaryBorder}`, borderRadius: 10, padding: 20, marginBottom: 16} }}>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            {form.role && (
              <div style={{ fontSize: 11, color: C.textMuted, marginTop: 4 }}>
                {{
                  manager:       'Receives ops alerts (with My Account → Manager phone and other active managers)',
              ...
              <div key={emp.id} style={{ background: C.cardBg, border: `0.5px solid ${isEditing ? C.primary : C.border}`, borderRadius: 10, overflow: 'hidden', transition: 'border-color .15s' }}>
                ...
                {isEditing && (
                  <div style={{ borderTop: `0.5px solid ${C.border}`, background: C.primaryLight, padding: '16px 16px 14px' }}>
                    ...
                    {isSelfOwner && (
                      <div style={{
                        fontSize: 12, color: C.primaryDark, marginBottom: 12, padding: '8px 10px',
                        background: C.cardBg, borderRadius: 8, border: `0.5px solid ${C.primaryBorder}`,
                      }}>
                        Edit your login email and personal phone in <Link to="/account" style={{ fontWeight: 600, color: C.primaryDark }}>My Account</Link> (top-left logo).
                      </div>
                    )}
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
                      ...
                      <div>
                        ...
                        {editForm.role && (
                          <div style={{ fontSize: 11, color: C.textMuted, marginTop: 3 }}>
                            {{
                              manager:       'Receives ops alerts (My Account → Manager phone + active managers)',
      ...
      {activeSection === 'waba' && (
        <div>
          ...
          <div style={{ background: '#FFF8EC', border: '0.5px solid #F4D78A', borderRadius: 8, padding: '10px 14px', marginBottom: 16, fontSize: 12, color: '#7A5A00' }}>
            💡 The shared access token for this WABA is set per-outlet via My Account → WhatsApp on each outlet.
          </div>
          <button onClick={saveBrand} disabled={saving}
            style={{ padding: '8px 22px', background: saving ? '#aaa' : C.primary, color: '#fff', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 500, cursor: saving ? 'default' : 'pointer' }}>
...
export default function SettingsPanel() {
  ...
  // WhatsApp management moved to My Account — preserve deep links
  if (searchParams.get('tab') === 'whatsapp') {
    const path = searchParams.get('path');
    const qs = path ? `?tab=whatsapp&path=${encodeURIComponent(path)}` : '?tab=whatsapp';
    return <Navigate to={`/account${qs}`} replace />;
  }
